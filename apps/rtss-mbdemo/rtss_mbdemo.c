...
static void rtss_mb_print_help(const char *prog_name)
{
	...
	rtss_log_info("Usage: %s [OPTIONS]\n\n", prog_name);
	rtss_log_info("Options:\n");
	rtss_log_info("  -s <string>      Input string (Max length: %zu bytes, Default String: %s)\n", DEMO_MSG_SZ, DEMO_DEFAULT_MSG);
	rtss_log_info("  -n <count>       Number of iterations per active core (Default count: %u)\n", DEMO_DEFAULT_ITERATIONS);
	rtss_log_info("  -c <mask>        Bitmask of cores to run (0x01-0x%02X, Default: 0x%02X = all cores)\n", DEMO_ALL_CORES_MASKED, DEMO_ALL_CORES_MASKED);
	rtss_log_info("  -h               Show this help message\n\n");
	rtss_log_info("Examples:\n");
	rtss_log_info("  %s                                   # Runs with default values (all cores)\n", prog_name);
	rtss_log_info("  %s -h\n", prog_name);
	rtss_log_info("  %s -c 0x01 -s \"Hello RTSS\" -n 10    # Core 0 only\n", prog_name);
	rtss_log_info("  %s -c 0x07 -s \"Hello RTSS\" -n 10    # Cores 0,1,2\n", prog_name);
	rtss_log_info("  %s -c 0x0F -s \"Hello RTSS\" -n 10    # All 4 cores\n\n", prog_name);
}
/* ========== Function: Select Channel Names ========== */
static int rtss_mb_select_channels(struct DemoAppConfig_t *config, int core)
{
	static const char * const tx_channels[] = {
		DEMO_C0_TX_CHANNEL,
		DEMO_C1_TX_CHANNEL,
		DEMO_C2_TX_CHANNEL,
		DEMO_C3_TX_CHANNEL
	};
	static const char * const rx_channels[] = {
		DEMO_C0_RX_CHANNEL,
		DEMO_C1_RX_CHANNEL,
		DEMO_C2_RX_CHANNEL,
		DEMO_C3_RX_CHANNEL
	...
	if (core < 0 || core > (int)DEMO_MAX_CORE) {
		rtss_log_err("Invalid core: %d\n", core);
		return EXIT_FAILURE;
	}
	ret = snprintf(config->tx_channel, DEMO_CHANNEL_NAME_SIZE, "%s", tx_channels[core]);
	if ((ret < 0) || ((size_t)ret >= DEMO_CHANNEL_NAME_SIZE)) {
		rtss_log_err("snprintf failed, ret=%d channel='%s'\n", ret, tx_channels[core]);
		return EXIT_FAILURE;
	}
	ret = snprintf(config->rx_channel, DEMO_CHANNEL_NAME_SIZE, "%s", rx_channels[core]);
	if ((ret < 0) || ((size_t)ret >= DEMO_CHANNEL_NAME_SIZE)) {
		rtss_log_err("snprintf failed, ret=%d channel='%s'\n", ret, rx_channels[core]);
		return EXIT_FAILURE;
	}
	return EXIT_SUCCESS;
}
...
static void rtss_mb_cleanup_resources(struct DemoAppConfig_t *Config)
{
	...
	if (!Config) {
		rtss_log_err("Invalid input parameters\n");
		return;
	}
	...
	if (ret != EXIT_SUCCESS) {
		rtss_log_err("Couldn't close channels, ret=%d\n", ret);
		return;
	}
	rtss_log_info("Cleanup complete\n");
}
/* ========== Command Parser ========== */
static int rtss_mb_command_parser(int argc, char *argv[], struct DemoAppConfig_t *config)
{
	...
	while ((opt = getopt(argc, argv, "hc:s:n:")) != -1) {
		...
		case 'c': {
			...
			if (errno != 0 || *endptr != '\0') {
				rtss_log_err("Invalid core mask: %s\n", optarg);
				return EXIT_FAILURE;
			}
			if ((config->core_masked <= (int)DEMO_MIN_CORE) ||
			    (config->core_masked > (int)DEMO_ALL_CORES_MASKED)) {
				rtss_log_err("Core mask must be between 0x%02X and 0x%02X (got: 0x%X)\n",
		...
		case 's': {
			...
			if (len > DEMO_MSG_SZ) {
				rtss_log_err("Invalid Message length (Max %zu Bytes), len=%zu\n", DEMO_MSG_SZ, len);
				return EXIT_FAILURE;
			}
			if (config->msg_allocated) {
				free(config->message);
			...
			if (config->message == NULL) {
				rtss_log_err("config->message calloc failed, len=%zu\n", len);
				return EXIT_FAILURE;
			}
		...
		case 'n': {
			...
			if (errno != 0 || *endptr != '\0') {
				rtss_log_err("Invalid iterations entry: %s\n", optarg);
				return EXIT_FAILURE;
			}
			if (val < 0 || val > (long)UINT_MAX) {
				rtss_log_err("Iterations out of range, val=%ld\n", val);
				return EXIT_FAILURE;
			}
...
int main(int argc, char *argv[])
{
	...
	rtss_log_info("Selected Core Mask : 0x%02X\n", (unsigned int)demo_config.core_masked);
	rtss_log_info("Input String       : %s\n", demo_config.message);
	rtss_log_info("Iterations         : %u\n\n", demo_config.iterations);

	ret = rtss_mb_open_channel(&demo_config);
	if (ret != EXIT_SUCCESS) {
		rtss_log_err("Open Channel Err %d\n", ret);
		ret = EXIT_FAILURE;
		goto Exit;
	}
	for (unsigned int count = 0; count < demo_config.iterations; count++) {
		if (g_signal_received) {
			rtss_log_info("\n\rSignal received, exiting gracefully...\n");
			ret = EXIT_SUCCESS;
			goto Exit;
		}
		for (int core = DEMO_MIN_CORE; core <= (int)DEMO_MAX_CORE; core++) {
			...
			if (ret < 0) {
				rtss_log_err("Rx Err %d, core=%d\n", ret, core);
				ret = EXIT_FAILURE;
				goto Exit;
			}
			rtss_log_info("%u [core%d] Recd: >%s<\n", count + 1, core, (char *)RxMsg.payload);
			/* Demo Wait */
			usleep(DEMO_DELAY_US);
		}
	}
