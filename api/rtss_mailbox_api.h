...
/**
 * struct rtss_mb_handle - handle context for a single mailbox sub-channel.
 * @fd:                File descriptor for /dev/rtssmb — kept open for the
 *                     lifetime of the channel. Closed by rtss_mb_close().
 *                     The mmap VMA is tied to this fd; no explicit munmap()
 *                     is needed — the kernel invalidates the VMA on close().
...
struct rtss_mb_handle {
	int		fd;
	int8_t		*p_mb_base_addr;	/* mmap base = descriptor base (KMD pre-adjusts addr) */
 ...
 * @handle:    Output. Caller-allocated; populated with channel context on success.
 * @dev: Name of the sub-channel to open (must match a name in the mailbox descriptor).
 *
 * Opens /dev/rtssmb, issues IOCTL to set the mailbox address, mmaps the mailbox
 * region into userspace, validates the mailbox descriptor, and for RX channels
 * registers an eventfd for interrupt notification. The fd is kept open and stored
 * in handle->fd — do not close it externally.
 *
 ...
 *
 * Return: Number of bytes written on success, RTSS_MB_RETURN_ERROR on failure.
 */
 ...
 *
 * Return: Number of bytes read on success, RTSS_MB_RETURN_ERROR on failure.
 */
 ...
 * @handle: Channel context from rtss_mb_open().
 *
 ...
 * No explicit munmap() is needed or performed.
 *
 * API contract: do not call rtss_mb_close() concurrently with rtss_mb_read()
 * or rtss_mb_write() on the same handle handle from another thread. The caller
 * is responsible for ensuring all read/write operations have completed before
 * closing. Same contract as fclose() vs fread()/fwrite() on a FILE*.
 *
 * Return: RTSS_MB_RETURN_SUCCESS on success, RTSS_MB_RETURN_ERROR on failure.
 */
