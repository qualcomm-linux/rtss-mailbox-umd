...
| Output | Location |
|--------|----------|
| `librtss_mailbox.so.1` | `build/lib/` |
| `librtss_safemlib.so.1` | `build/lib/` |
| `librtss_ota.so.1` | `build/lib/` |
| `rtssdbg` | `build/apps/rtss-dbg/` |
| `rtss_console` | `build/apps/rtss-console/` |
| `rtss_mbdemo` | `build/apps/rtss-mbdemo/` |
| `rtss_ota` | `build/apps/rtss-ota/demo/` |
...
3. Add `FILES:<pkg> += "${bindir}/<your-binary>"` to the appropriate package in
   `layers/meta-qti-rtss-mailbox-umd/recipes-rtss-mailbox-umd/qcom-rtss-mailbox-umd/qcom-rtss-mailbox-umd.bb`.
   ...
   sub-package (following the `qcom-rtss-ota` pattern) is appropriate.
