...
int rtss_mb_open(struct rtss_mb_handle **handle, const char *dev)
{
	...
	if (handle == NULL) {
		RTSS_MB_ERR("invalid handle pointer\n");
		return -EINVAL;
	}
	...
	if (h == NULL) {
		RTSS_MB_ERR("[%s] handle alloc failed\n", dev);
		return -ENOMEM;
	}

	/* Sentinels — calloc zeroes but set explicitly for clarity. */
	h->fd       = -1;
	h->event_fd = -1;
	...
	*handle = h;
	...
	RTSS_MB_DBG("[%s] open ok (addr=0x%llx size=0x%x)\n",
